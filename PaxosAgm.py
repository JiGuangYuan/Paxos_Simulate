        ...
        while True:
            ...
            if var["type"] == "request":
                ...
                rsp = {
                    ...
                    "acceptors": acceptors_random  # 表决者编号
                }
                self.value_num += 1
                self.value_index += 1
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # 没有接受到消息
                if self.start_propose is True and time.time() - self.time_start > 5:
                    print_str(self.name + "的本轮决议" + self.value + "投票结束，同意:" + str(self.accept) + "拒绝：" + str(
                        self.reject) + "选择:" + str(self.chosen))
                    self.start_propose = False
                    if self.reject > 0:
                        print_str(self.name + "的决议" +
                                  self.value + "被否决，停止提议，退出")
                    if self.chosen == len(self.acceptors):
                        print_str("############# " + self.name + "的决议" + self.value + "被同意，完成决议过程 #############")
                    if (self.accept > 0 or
    ...
    def run(self):
        while True:
            try:
                ...
                var_dict = self.process_propose(var)
                # 有2%的概率发送失败
                if random.randrange(100) < 80:
                    self.queue_to_proposers[var["proposer"]].put(var_dict)
                else:
...
if __name__ == '__main__':
    acceptors_num = 20  # acceptors的数量
    proposers_num = 5  # proposers的数量
